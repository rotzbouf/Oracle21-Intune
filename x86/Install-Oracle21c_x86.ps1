...
<#
.SYNOPSIS
    Installs Oracle Database Client 21c (32-bit / Runtime) — Intune Win32 bundled deployment.

.DESCRIPTION
    All files travel inside the .intunewin package:
        setup.exe + Oracle installer files  ← extracted Oracle x86 installer
        client_install_x86.rsp              ← silent install response file
        tnsnames.ora                        ← your custom file
        sqlnet.ora                          ← your custom file
        Install-Oracle21c_x86.ps1  (this file)
    ...
    Intune Win32 app settings:
        ...
        Detection         : Custom script — Detect-Oracle21c_x86.ps1  /  Run as 32-bit: Yes
        Max install time  : 120 minutes  ← increase from the default 60 min
        Dependencies      : Oracle 21c Client (x64)  [auto-install]
#>
...
try {
    Write-Log "=== Oracle 21c Client x86 — Install START ==="
    ...
    # ── Dependency guard ──────────────────────────────────────────────────────
    # x64 should be installed first (creates the shared oraInventory).
    # Intune handles this via the Dependencies tab, but we log a warning just in case.
    if (-not (Test-Path 'C:\Oracle\product\21.0.0\client_x64\bin\sqlplus.exe')) {
        Write-Log "WARNING: Oracle x64 client not detected. Install x64 first." 'WARN'
    }

    # ── Already installed? ────────────────────────────────────────────────────
    if (Test-Path "$OracleHome\bin\sqlplus.exe") {
        Write-Log "Oracle x86 already installed — skipping setup.exe, updating .ora files only." 'WARN'
    } else {

        # ── Silent install ────────────────────────────────────────────────────
        Write-Log "Running Oracle OUI silent install (32-bit)..."
    ...
    # ── PATH note ─────────────────────────────────────────────────────────────
    # x86 bin is intentionally NOT added to the global PATH to avoid DLL conflicts
    # with the x64 client. Uncomment below only if your 32-bit apps require it.
    #
    ...
    Write-Log "=== Oracle 21c Client x86 — Install SUCCESSFUL ==="
