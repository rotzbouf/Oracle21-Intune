...
<#
.SYNOPSIS
    Uninstalls Oracle Instant Client ODBC driver (32-bit) — Intune Win32.
#>

$InstallDir = 'C:\Oracle\instantclient_21_x86'
$LogDir     = 'C:\ProgramData\Microsoft\IntuneManagementExtension\Logs'
$LogFile    = "$LogDir\OracleODBC_x86_Uninstall.log"
...
try {
    Write-Log "=== Oracle ODBC Driver 21c x86 — Uninstall START ==="
    ...
    if (Test-Path $odbcUninstall) {
        ...
        $refKey = 'HKLM:\SOFTWARE\WOW6432Node\ODBC\ODBCINST.INI\ODBC Drivers'
        if (Test-Path $refKey) {
            Remove-ItemProperty -Path $refKey -Name 'Oracle in instantclient_21_x86' -ErrorAction SilentlyContinue
        }
    }
    ...
    Write-Log "=== Oracle ODBC Driver 21c x86 — Uninstall COMPLETE ==="
    Stop-Transcript; exit 0
