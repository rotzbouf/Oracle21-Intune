...
<#
.SYNOPSIS
    Installs Oracle Instant Client 21c + ODBC driver (64-bit) — Intune Win32 deployment.

.DESCRIPTION
    Extracts and registers the Oracle 64-bit ODBC driver for use by 64-bit applications
    (Excel 64-bit, Power BI Desktop, 64-bit ODBC data sources, etc.)

    No Oracle Universal Installer (OUI) is used — extraction + odbc_install.exe only.
    Much lighter than the full Runtime client (~25-40 MB .intunewin vs ~600 MB).

    Package folder layout:
        instantclient-basic-windows.x64-21.*.zip   ← Oracle Instant Client Basic x64
        instantclient-odbc-windows.x64-21.*.zip    ← Oracle Instant Client ODBC x64
        tnsnames.ora                               ← your file
        sqlnet.ora                                 ← your file
        Install-OracleODBC_x64.ps1  (this file)
        Uninstall-OracleODBC_x64.ps1
        Detect-OracleODBC_x64.ps1

    Download the two ZIPs (free, no login required) from:
        https://www.oracle.com/database/technologies/instant-client/winx64-64-downloads.html
        → "Basic Package"  : instantclient-basic-windows.x64-21.x.x.x.zip
        → "ODBC Package"   : instantclient-odbc-windows.x64-21.x.x.x.zip
    ...
    Intune Win32 app settings:
        ...
        Detection        : Custom script — Detect-OracleODBC_x64.ps1  /  Run as 32-bit: No
        Max install time : 30 minutes
        Dependencies     : VCRedist 2015-2022 (x64) — Auto Install: Yes
#>

# ── Configuration ─────────────────────────────────────────────────────────────
$InstallDir   = 'C:\Oracle\instantclient_21_x64'   # fixed install path on client
$TnsAdmin     = $InstallDir                         # tnsnames.ora lives here
$ScriptDir    = Split-Path -Parent $MyInvocation.MyCommand.Definition
$LogDir       = 'C:\ProgramData\Microsoft\IntuneManagementExtension\Logs'
$LogFile      = "$LogDir\OracleODBC_x64_Install.log"
$TempDir      = "$env:SystemRoot\Temp\OracleODBC_x64_Extract"
# ─────────────────────────────────────────────────────────────────────────────
...
function Expand-ZipTo {
    # Extracts a ZIP's root-folder contents into $Destination (handles merging safely)
    param([string]$ZipPath, [string]$Destination)
    $tempExtract = "$TempDir\_zip_$(Split-Path -Leaf $ZipPath)"
    New-Item -ItemType Directory -Path $tempExtract -Force | Out-Null
    Expand-Archive -Path $ZipPath -DestinationPath $tempExtract -Force

    # Oracle ZIPs contain a single top-level folder (e.g. instantclient_21_12)
    # Copy its contents into the target, merging with any existing files
    $icRoot = Get-ChildItem -Path $tempExtract -Directory | Select-Object -First 1
    if (-not $icRoot) {
        # ZIP has no subfolder — contents are at root level
        $icRoot = Get-Item $tempExtract
    }
...
try {
    Write-Log "=== Oracle ODBC Driver 21c x64 — Install START ==="

    # ── Find ZIPs in package folder ───────────────────────────────────────────
    $basicZip = Get-ChildItem -Path $ScriptDir -Filter 'instantclient-basic-windows.x64-21*.zip' |
                Select-Object -First 1
    $odbcZip  = Get-ChildItem -Path $ScriptDir -Filter 'instantclient-odbc-windows.x64-21*.zip' |
                Select-Object -First 1

    if (-not $basicZip) {
        Write-Log "Basic ZIP not found in $ScriptDir (pattern: instantclient-basic-windows.x64-21*.zip)" 'ERROR'
        Stop-Transcript; exit 1
    }
    if (-not $odbcZip) {
        Write-Log "ODBC ZIP not found in $ScriptDir (pattern: instantclient-odbc-windows.x64-21*.zip)" 'ERROR'
        Stop-Transcript; exit 1
    }
    ...
    if (Test-Path $driverKey) {
        ...
        # ── Prepare install dir and temp dir ──────────────────────────────────
        ...
        # ── Verify odbc_install.exe is present ────────────────────────────────
    ...
    # ── Add InstallDir to system PATH (required for DLL resolution) ───────────
    ...
    Write-Log "=== Oracle ODBC Driver 21c x64 — Install SUCCESSFUL ==="
